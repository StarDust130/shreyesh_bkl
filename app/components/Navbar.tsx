...
export default function Navbar() {
  return (
    <nav className="fixed top-0 left-0 w-full z-50 py-4 bg-background/80 backdrop-blur-sm border-b border-black/5">
      <Container className="flex items-center justify-between">
        ...
        <div className="hidden md:flex items-center gap-8 text-sm font-medium tracking-wide text-gray-600">
          {['Index', 'Service', 'Classes', 'Contact', 'Blog', 'About Us'].map((item) => (
            <Link key={item} href="#" className="hover:text-red-600 transition-colors">
              {item}
            </Link>
          ))}
        </div>
